...
class RAGAttributionEngine:
  ...
  def fetch_web3_crypto_intelligence(self):
    ...
    data = [
        {
            "title": (
                "Agentic Finance 重新平衡 RWA 流動性池，減半後礦企啟動資產轉型"
            ),
            "ai_sentiment": "Bullish",
            "vol_multiplier": "3.5x",
            "source": "CoinDesk",
  ...
  def fetch_tw_stocks_intelligence(self):
    ...
    data = [
        {
            "title": (
                "經濟部研議爭取美方 AI 資料中心來台落腳，推動亞洲算力與綠電樞紐計畫"
            ),
            "ai_sentiment": "Bullish",
            "vol_multiplier": "3.8x",
            "source": "台灣經濟部 (MOEA)",
  ...
  def fetch_global_vix(self):
    """🌐 動態抓取即時 VIX 指數，並自動同步卡片數值與下方說明文案"""
    try:
      vix = yf.Ticker("^VIX")
      hist = vix.history(period="1d")
      vix_value = float(hist["Close"].iloc[-1]) if not hist.empty else 17.8
    except Exception:
      vix_value = 17.8  # 若抓取失敗時的預設 fallback 值

    vix_rounded = round(vix_value, 2)
    current_month_str = datetime.now().strftime("%Y.%m")

    # 根據 VIX 數值動態切換狀態與文案，確保上下數值與說明完全一致
    if vix_rounded > 20:
      status_zh = f"突破關鍵20關卡 / 強烈避險情緒 (Risk-Off)"
      ...
      status_zh = f"平穩區間 / 資本流向穩健 (Neutral/Risk-On)"
