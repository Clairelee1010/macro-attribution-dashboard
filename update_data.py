...
class RAGAttributionEngine:
    """
    RAG 跨市場大腦數據生成器
    確保輸出的 JSON Key 與前端 index.html 完全一致：
    - us_macro
    - web3_crypto
    - tw_stocks
    - global_vix
        ...
        return [
            {
                ...
                "macro_attribution": "【Agentic Finance & RWA 深度歸因】2026 減半週期過後，純挖礦邊際利潤下滑。AI 智能體金融全面接管鏈上資產配置，頭部礦企啟動資產代幣化（RWA）戰略，鎖定長期法幣實質現金流。",
                "tw_spillover_effect": "美股 Web3 概念股全面翻新重電與機房設備，這股 AI 數據中心與資產轉型潮，直接轉化為台廠高階 AI 伺服器代工與液冷散熱的爆發性實質訂單。",
                "rwa_flow_metric": "鏈上基礎設施 RWA 資金池 24H 淨流入達 +$45.8M，多鏈智能體自動結算量創歷史新高。",
                "agent_action_log": "[AGENT_FINANCE] Protocol executing autonomous cross-chain settlement.",
                "recommended_groups_tw": [{"name": "AI 先進製程與伺服器代工（台積電 2330、廣達 2382）", "reason": "通吃美股巨頭與礦企轉型 AI 的硬體升級紅利。"}],
                "high_risk_groups_tw": [{"name": "傳統低階顯卡、純挖礦板卡廠", "reason": "資金向 AI 高階晶片靠攏，傳統挖礦硬體面臨需求邊緣化。"}],
                "recommended_groups_us": [{"name": "MARA Holdings (MARA), NVIDIA (NVDA)", "reason": "基礎設施轉型速度領先同業，算力晶片剛性需求明確。"}],
                "high_risk_groups_us": [{"name": "Riot Platforms (RIOT)", "reason": "若轉型 AI 算力速度落後同業，純挖礦利潤在減半週期後將持續承壓。"}]
            }
        ]
        ...
        return [
            {
                ...
                "macro_attribution": "【台股政策歸因】經濟部最新政策聚焦於提供穩定的高階算力基礎設施與綠電配套，積極吸引美股科技巨頭與進行 AI 轉型的 Web3 礦企將亞太 AI 資料中心落地台灣。",
                ...
                "agent_action_log": "[POLICY_AGENT] High-priority keyword correlation detected: Green Energy Hub.",
    ...
    def generate_dashboard_data(self):
        return {
            ...
            "global_vix": {
                ...
                "insight_zh": "【VIX 跨境聯動歸因】VIX 恐慌指數飆升至 22.4，反映宏觀資金正全面買入波動率期權進行下行保護。此現象與鏈上美債 RWA 池（+$45.8M）資金流入高度共振。",
                "insight_en": "【VIX Cross-Market Insight】VIX jumps to 22.4, signaling widespread institutional hedging via volatility options."
            }
        }
...
if __name__ == "__main__":
    ...
    # 【重要】請確認你的 index.html fetch 的是 'data.json' 還是 'market_data.json'
    # 這裡預設寫入 'data.json'，如果是其他檔名請自行修改下方字串
    output_filename = 'data.json'
    
    with open(output_filename, 'w', encoding='utf-8') as f:
        json.dump(final_data, f, ensure_ascii=False, indent=4)
        
    print(f"✅ 數據已成功更新至 {output_filename}！Key 值結構已完美對齊前端網頁。")
